...
# Loading in blastp hits
d = paste0(dat_dir, cruise, "/")
hits <- read.csv(paste0(d, cruise, "_topHit_wTax.csv")) %>% tibble %>% 
     rename(taxID = taxid, TargetName = qseqid)

hits_plot <- hits %>%
     mutate(#e = ifelse(evalue <= 0 | is.na(evalue), .Machine$double.xmin, evalue),
            mlog10_e = -log10(evalue))
...
taxIDs <- hits %>% select(taxID) %>% distinct
...
ncbi_taxa2 <- ncbi_taxa %>%
     mutate(across(all_of(ranks), ~na_if(., "")))
...
cutoffs <- read.csv(paste0(dat_dir, "query_eValues.csv")) %>% tibble
ncbi_taxa2
...
write.csv(cutoff_summary, paste0(metaT_dir, cruise, "_cutOff_summary.csv"), row.names = F)
