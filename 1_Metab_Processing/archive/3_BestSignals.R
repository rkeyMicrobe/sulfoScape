...
hp <- read.csv("data/ATL_GOM/QCd/QEQC_GOM_ATL_HILIC_Pos_exp6Clean.csv")
hn <- read.csv("data/ATL_GOM/QCd/QEQC_GOM_ATL_HILIC_Neg_exp8Clean.csv")
rp <- read.csv("data/ATL_GOM/QCd/QEQC_GOM_ATL_RP_expClean3.csv")
...
mixClean <- function(data = NULL) {
     mixKey <- read.csv("data/1_metaData/metab_MixSets.csv") %>% tibble %>% 
          select(Compound_Name, mixSet_2021) %>% 
          drop_na(mixSet_2021) %>% distinct
     
     data %>%  drop_na(Compound_Name) %>% 
          left_join(mixKey, by = "Compound_Name") %>%
          filter(
               str_detect(sampleID, "Std_H2OinMatrix") |
                    (str_detect(sampleID, "Mix1") & mixSet_2021 == "Mix1") |
                    (str_detect(sampleID, "Mix2") & mixSet_2021 == "Mix2")
          )
}
hp <- mixClean(data = hp)
hn <- mixClean(data = hn)
...
getSums <- function(data = NULL, sepType = NULL){
     x <- data %>% drop_na(Compound_Name) %>% 
          select(sampleID, Compound_Name, areaRAW) %>% 
          filter(str_detect(sampleID, "Std_")) %>% 
          distinct %>% 
          pivot_wider(., names_from = Compound_Name, values_from = areaRAW)
...
rk <- bests_cleaned %>% select(metab, bestMode) %>% distinct
