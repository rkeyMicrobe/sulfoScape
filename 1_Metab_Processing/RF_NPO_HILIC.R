...
# LOAD DATA INPUTS
posBMIS <- read.csv("data/NPO_G2/bmis/BMIS_NPO_G2_HP_normMetabs.csv")
preBMIS <- read.csv("data/NPO_G2/QCd/QEQC_HILIC_Pos5.4_2017-2019_allSulfurStds.csv") 
method      = "HP" # Choices: HP or HN
Mode        = "Pos" # Pos or Neg
...
std_mixSets <- read.csv("data/1_metaData/metab_MixSets.csv")
...
mixKey <- std_mixSets %>% filter(ionMode == Mode) %>% 
     select(Compound_Name, ionMode, conc_uM_2019, mixSet_2019) %>% 
     rename(HILIC_Mix = mixSet_2019, Concentration_uM = conc_uM_2019) %>% 
     distinct 
