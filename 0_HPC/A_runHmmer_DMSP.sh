...
NAME="G2NS"
GENE=$(sed -n "${SLURM_ARRAY_TASK_ID}p" "${PROJ}/geneList_DMSP.txt")
QUERY="${PROJ}/genes/DMSPgenes/${GENE}.hmm"
...
#echo "(This is from the modified runHmmer_multiGene.sh script)"
module purge; module load hmmer
#hmmbuild "${PROJ}/genes/DMSPgenes/${GENE}.hmm" "${OUTDIR}/${GENE}.sto"
...
hmmsearch --cpu 2 \
  ...
  "${PROJ}/genes/DMSPgenes/${GENE}.hmm" "$CONTIG" \
  > "${OUTDIR}/${GENE}.out"
...
HEADER="TargetName,QueryName,Evalue,BitScore,Bias,ExpNumDomains,BestDomEvalue,BestDomScore,BestDomBias,Description"

# Filtered output
echo "$HEADER" > "$OUTCSV"
grep -v '^#' "${OUTDIR}/${GENE}_filtered.tbl" | \
awk -v gene="${GENE}" 'BEGIN {OFS=","} {print $1, gene, $5, $6, $7, $12, $9, $10, $11, $13}' \
>> "$OUTCSV"
