...
tax <- read.csv(paste0(d, cruise, "_hitTaxonomy2.csv")) %>% tibble
...
tax_clean <- tax %>% filter(nt_id %in% list) %>% 
     rename(contig = nt_id, taxID = tax_id) %>% 
...
# Save Files
svg(filename = paste0(plo_dir, cruise, "_numberNA_LCA.svg"), width = 7, height = 5, bg = "transparent")
p1; dev.off()
